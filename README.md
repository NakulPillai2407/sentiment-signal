...
An interactive Streamlit app that pulls live news for a user-chosen set of tickers, scores it with FinBERT (a finance-tuned transformer), pulls matching price data, and tests whether sentiment has any measurable relationship with subsequent abnormal returns using proper OLS inference rather than a bare correlation. It also includes a long/short backtest of the sentiment signal, and states the statistical limits of what the results can support directly in the app.

Built as a portfolio project for Quantitative Analyst / Financial Technology roles. Originally a university Computational Economics coursework script, rebuilt here as a live, interactive tool.

## Live Demo

[https://nakul-sentiment-signal.streamlit.app/](https://nakul-sentiment-signal.streamlit.app/)

## Overview

This project started as a coursework script for a Computational Economics module. The original version used NLTK's VADER (a general-purpose, social-media-tuned lexicon model) to score financial headlines, ran on a static, one-time pull of 13 hand-picked FTSE 100 tickers with 10 articles each, computed a single daily return as the percentage change between the closing price roughly 2 days before and after an article's publish date (with no separation of market-wide vs. firm-specific price movement), and regressed sentiment against that raw return with `sklearn`, getting a slope of about 1.04 and an R² of about 0.02. One obvious outlier was handled by hardcoding `df.drop(80)`.

This app rebuilds the pipeline end to end: a finance-tuned sentiment model in place of a general-purpose one, an event-study return methodology that separates market-wide moves from firm-specific ones, and proper statistical inference in place of a slope and an R².
...
- Includes an illustrative sentiment-threshold long/short backtest, explicit about its lack of transaction costs, slippage, and small sample size
...
- One-time fetch per session via `st.session_state`, so switching tabs or tweaking a slider never re-hits the network
...
Abnormal returns are used because a stock's raw daily return is mostly driven by how the whole market moved that day, not by firm-specific news. A market-wide 2% rally will show up as a positive return on every stock in the universe regardless of whether that stock had any news at all. Abnormal return subtracts out the market's move (flat subtraction of the benchmark's return, or a beta-scaled version via a simple rolling market-model regression) to isolate the part of the move plausibly attributable to the stock's own news. See `analysis/returns.py` for the full derivation and the trade-offs between the two approaches.

FinBERT is used over VADER because VADER is a lexicon model built for social media text with no notion of financial-domain meaning; phrases like "cuts costs by reducing headcount" can score as neutral-to-positive under a general lexicon while being bearish in context. FinBERT (`ProsusAI/finbert`) is BERT further pre-trained on financial text and fine-tuned on analyst-labelled sentiment, so it has actually seen domain-specific usage. VADER is kept in the app specifically so the Model Diagnostics tab can show this gap empirically (agreement rates, side-by-side regression strength) rather than just asserting it.

`statsmodels` is used over `sklearn` because `sklearn.LinearRegression` gives a fitted line and nothing else; it's built for prediction. `statsmodels.OLS` gives standard errors, t-statistics, p-values, and confidence intervals, which is what's needed to answer whether a relationship is distinguishable from noise, given a small sample, rather than just what the best-fit line looks like.
...
- Sector-level breakdown: does the sentiment-return relationship differ by sector (e.g. cyclicals vs. defensives)?
- A real risk-free rate series for the Sharpe ratio instead of a flat approximate rate, and proper trading-calendar-aware annualization instead of simple period-count scaling.
...
- A larger, longer-running sample collected over weeks/months rather than a single on-demand pull. Sample size is the single biggest limitation of the current version, and this would materially improve the statistical power of every regression.
- Independent verification of news timestamp integrity, to rule out the lookahead-bias risk flagged in the Limitations tab (checking whether Yahoo Finance summaries are ever edited post-publication).
